...
class PrivateKey(object):
    def __init__(self, p=None, g=None, x=None):
        ...
        self.g = g
        self.x = x
        self.bit_len = self.p.bit_length()
...
def verify(r, s, pub_key, message:str):
    if r < 0 or r >= pub_key.p:
        return False
    if s < 0 or s > pub_key.p - 1:
        return False
    h_pow = pow(pub_key.g, get_hash_int(message), pub_key.p)
    sg = mul_mod(pow(pub_key.h, r, pub_key.p), pow(r, s, pub_key.p), pub_key.p)
